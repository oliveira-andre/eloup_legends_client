    ...
    <div className="flex h-screen overflow-hidden services-container">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-[#091428]/50 flex items-center justify-between px-8 border-b border-white/5">
          <h1 className="text-2xl font-bold text-white">Catálogo de Jobbers / Boosters</h1>

          <Link href="/jobbers/new" className="bg-lol-gold hover:bg-yellow-600 text-black font-bold py-2 px-6 rounded flex items-center gap-2 transition transform active:scale-95 shadow-lg shadow-lol-gold/20">
            <i className="fa-solid fa-plus"></i>
            <span className="hidden sm:inline">Novo Jobber / Booster</span>
          </Link>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <p className="text-gray-400 text-sm">Mostrando <span className="text-white font-bold">{jobbersData.length} jobbers</span> ativos na plataforma.</p>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-black/40 text-gray-400 text-xs uppercase tracking-widest border-b border-white/5">
                  ...
                  <th className="px-6 py-5 hidden lg:table-cell">Elo / Rank</th>
              ...
              <tbody className="divide-y divide-white/5">
                ...
                {jobbersData.map((jobber: Jobber) => (
                  <tr className="hover:bg-white/5 transition group" key={jobber.id}>
                    <td className="px-6 py-4">
                      <img src={`${process.env.NEXT_PUBLIC_API_URL}/${jobber.picture}`}  className="service-img" alt={jobber.name} />
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-white group-hover:text-lol-gold transition">{jobber.position}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-white group-hover:text-lol-gold transition">{jobber.name}</div>
                    </td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <div className="font-bold text-white group-hover:text-lol-gold transition">{jobber.elo.name}</div>
                      {jobber.elo.has_rank && (
                        <div className="text-[10px] bg-lol-blue/10 text-lol-blue px-2 py-0.5 rounded inline-block mt-1">Rank {jobber.rank}</div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-gray-400 text-xs max-w-xs line-clamp-2">{jobber.observation}</p>
                    </td>
